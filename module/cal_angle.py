...
def calculate_knee_alignment(hip, knee, ankle, width, height):
    ...
    hip_xy = np.array(
        [hip[0] * width, hip[1] * height],
        dtype=np.float32,
    )
    knee_xy = np.array(
        [knee[0] * width, knee[1] * height],
        dtype=np.float32,
    )
    ankle_xy = np.array(
        [ankle[0] * width, ankle[1] * height],
        dtype=np.float32,
    )
...
def calculate_all_features(landmarks, width, height, vis_th=0.5):
    ...
    features = {
        ...
        # ===== 측면 지표 =====
        "fha_deg": safe(
            ok("right_ear", "right_shoulder", "right_hip"),
    ...
    lambda: calculate_thoracic_kyphosis(
        ...
        "pelvic_tilt_ant_deg": safe(
            ok("right_shoulder", "right_hip", "right_knee"),
...
def features_to_metrics(front_features, side_features):
    ...
    metrics = {
        ...
        "thoracic_kyphosis": side_features.get(
            "thoracic_kyphosis_deg"
        ),
        "pelvic_tilt_ant": side_features.get(
            "pelvic_tilt_ant_deg"
        ),
    }
