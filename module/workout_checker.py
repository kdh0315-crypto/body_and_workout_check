...
#
# ===== 학습된 TRT 엔진 로드 (미사용) =====
# bicep_curl_model = TRTModel("bicep_curl_classifier.trt")
# plank_model      = TRTModel("plank_classifier.trt")
# squat_model      = TRTModel("squat_classifier.trt")   # 스쿼트는 규칙 기반이라 원래 미사용
        ...
        if not self.session.done:
            ...
            if self.current_state == self.STATE_BOTTOM:
                self.bottom_snapshot_errors = self._check_bottom_form(
                    trunk_angle, knee_angle, hip_flex_angle)
...
class BicepCurlChecker:
    ...
    TRUNK_CLEAR_OK = 8
    # MODEL_CONFIDENCE_THRESHOLD = 0.7   # TRT 미사용
    ...
    def _check_flexed_form(self, elbow_angle, trunk_angle):
        """TRT 미사용: trunk_angle이 허용치를 넘으면 상체 젖힘으로 판정."""
        errors = []
    ...
    def update(self, landmarks, w, h):
        ...
        if not self.session.set_completed:
            ...
            move_done = (self.current_state == self.STATE_EXTENDING
                        and new_state == self.STATE_EXTENDED)
...
def get_exercise_checker(exercise_name, target_reps=5, target_count=10, rest_seconds=30):
    if exercise_name == "squat":
        ...
        return PlankChecker(target_hold_seconds=target_count)
    else:
